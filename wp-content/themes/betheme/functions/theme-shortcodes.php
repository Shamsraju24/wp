		...
		if( is_numeric( $src ) ){
			...
			if( ! empty($lazy_load) && 'disable' == $lazy_load ){
				...
				add_filter( 'wp_lazy_loading_enabled', '__return_false' );
					$image_output_escapes = wp_get_attachment_image( $src, $size, false, [ 'class' => 'scale-with-grid', 'style' => $img_style ] );
				add_filter( 'wp_lazy_loading_enabled', $lazy_status );
			} else {
				$image_output_escapes = wp_get_attachment_image( $src, $size, false, [ 'class' => 'scale-with-grid', 'style' => $img_style ] );
			}

			// $image_output_escapes .= 'test';
			...
			$image_output_escapes = '<img class="'. esc_attr($class) .'" '. $src .' alt="'. esc_attr($alt) .'" title="'. esc_attr($title) .'" width="'. esc_attr($width) .'" height="'. esc_attr($height) .'" style="'. $img_style .'"/>';
